...
router.get('/', requireAuth, requireRol(...ROLES_ORG), async (req, res, next) => {
  try {
    ...
    if (estado !== undefined && !['verde', 'ambar', 'rojo'].includes(estado)) {
      return res.status(400).json({ error: 'estado debe ser verde, ambar o rojo' });
    }
...
router.get('/:id/historico', requireAuth, requireRol(...ROLES_ORG), async (req, res, next) => {
  try {
    ...
    if (limite === null) {
      return res.status(400).json({ error: 'limite debe ser un entero entre 1 y 1000' });
    }
  ...
  try {
    ...
    if (b.zona_id !== undefined) {
      if (b.zona_id === null) {
        ...
        if (!esId(b.zona_id)) return res.status(400).json({ error: 'zona_id invalido' });
        const zona = await db.query(
          `SELECT id FROM zonas WHERE id = $1 AND organizacion_id = $2`,
          [b.zona_id, org]
        );
        if (zona.rows.length === 0) return res.status(404).json({ error: 'Zona no encontrada' });
    ...
    if (b.altura_cm !== undefined) {
      ...
      if (!Number.isFinite(altura) || altura <= 0) {
        return res.status(400).json({ error: 'altura_cm debe ser mayor que 0' });
      }
    ...
    if (b.umbral_ambar !== undefined || b.umbral_rojo !== undefined) {
      ...
      if (!validarUmbrales(ambar, rojo)) {
        return res.status(400).json({ error: 'Umbrales invalidos: enteros de 0 a 100 y umbral_rojo mayor que umbral_ambar' });
      }
      poner('umbral_ambar', ambar);
      poner('umbral_rojo', rojo);
    }
    if (b.activo !== undefined) {
      if (typeof b.activo !== 'boolean') return res.status(400).json({ error: 'activo debe ser true o false' });
    ...
    if (b.device_id !== undefined) {
      ...
      if (b.device_id !== null && !deviceId) return res.status(400).json({ error: 'device_id invalido' });
    ...
    if (b.regenerar_token === true && tokenNuevo === null) {
      ...
      if (!deviceId) return res.status(400).json({ error: 'El contenedor no tiene device_id' });
    ...
    if (e.code === '23505') {
      return res.status(409).json({ error: 'Ya existe un contenedor con ese device_id' });
    }
    if (e.code === '23514') {
...
router.post('/', requireAuth, requireRol('admin'), async (req, res, next) => {
  try {
    ...
    if (!codigo || !nombre) {
      return res.status(400).json({ error: 'Faltan codigo o nombre' });
    }
    if (!coordenadasValidas(lat, lng)) {
    ...
    if (e.code === '23505') {
      return res.status(409).json({ error: 'Ya existe un contenedor con ese codigo o device_id' });
    }
    next(e);
  }
});
...
router.post('/inventario', requireAuth, requireRol('superadmin'), async (req, res, next) => {
  try {
    ...
    if (!codigo || !deviceId) {
      return res.status(400).json({ error: 'Faltan codigo o device_id' });
    }
    if (!Number.isFinite(altura) || altura <= 0) {
      return res.status(400).json({ error: 'altura_cm debe ser mayor que 0' });
    }
...
router.post('/vincular', requireAuth, requireRol('particular', 'admin', 'supervisor', 'operario'), async (req, res, next) => {
  try {
    ...
    if (!codigoVinculacion || !nombre) {
      return res.status(400).json({ error: 'Faltan codigo_vinculacion o nombre' });
    }
    if (!coordenadasValidas(lat, lng)) {
      return res.status(400).json({ error: 'lat/lng invalidos' });
    }
    ...
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Codigo de vinculacion invalido o ya utilizado' });
    }
    res.json(rows[0]);
  } catch (e) {
    if (e.code === '23505') {
      return res.status(409).json({ error: 'Ya existe un contenedor con ese codigo en tu organizacion' });
    }
    next(e);
  }
});
...
router.post('/inventario/lote', requireAuth, requireRol('superadmin'), async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(lista) || lista.length === 0 || lista.length > 1000) {
      return res.status(400).json({ error: 'dispositivos debe ser una lista de 1 a 1000 elementos' });
    }
    const altura = num(b.altura_cm);
    const { ambar, rojo, validos } = leerUmbrales(b);
    if (!Number.isFinite(altura) || altura <= 0) {
    ...
    for (const [i, d] of lista.entries()) {
      ...
      if (!codigo || !deviceId) {
        return res.status(400).json({ error: `dispositivos[${i}]: faltan codigo o device_id` });
      }
      if (codigos.has(codigo) || dispositivos.has(deviceId)) {
        return res.status(400).json({ error: `dispositivos[${i}]: codigo o device_id repetido en la lista` });
      }
      codigos.add(codigo);
    ...
    if (e.code === '23505') {
      return res.status(409).json({ error: 'Algun codigo o device_id ya existe; no se creo ninguno' });
    }
    next(e);
  }
});
